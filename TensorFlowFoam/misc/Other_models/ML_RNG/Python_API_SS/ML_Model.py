...
ML_DATA_DIR = PARENT+'/Data_Extraction_SS/'
...
def load_data():
    # Load data
    total_data = np.load(ML_DATA_DIR+'Total_Data_SS.npy')
...
def fit_model(training_inputs,training_outputs,model,num_epochs):
    ...
    earlystopping = keras.callbacks.EarlyStopping(monitor='val_loss', min_delta=0, patience=20, verbose=0, mode='auto', baseline=None, restore_best_weights=False)

    history = model.fit({'input_placeholder': training_inputs},
          {'output_value': training_outputs},
          epochs=num_epochs,
          batch_size=256,callbacks=[tbCallBack,checkpoint,csv_logger,earlystopping],validation_split=0.1,verbose=1)
    ...
    tf.train.write_graph(frozen_graph, './', 'ML_RNG.pb', as_text=False)

    graph = load_graph('ML_RNG.pb')
